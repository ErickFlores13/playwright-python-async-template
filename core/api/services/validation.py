...
from core.api.models import ValidationError
...
class ValidationService:
    ...
    def validate_schema(self, data: Any, model: Type[T]) -> T:
        """
        ...
        try:
            ...
            raise ValidationError(message=error_msg, response_body=data) from e

    def validate_status_code(
        self, response: APIResponseWrapper, expected: Union[int, List[int]]
    ) -> None:
        """
        ...
        if response.status_code not in expected_codes:
            error_msg = f"Expected status code {expected_codes} but got {response.status_code}"
            logger.error(error_msg)
            raise ValidationError(
                message=error_msg, status_code=response.status_code, response_body=response.data
            )
    ...
    def validate_json_path(self, data: Any, path: str, expected_value: Optional[Any] = None) -> Any:
        """
        ...
        try:
            ...
            if expected_value is not None and value != expected_value:
                error_msg = f"Value at path '{path}' is '{value}' but expected '{expected_value}'"
                logger.error(error_msg)
                raise ValidationError(message=error_msg, response_body=data)
    ...
    def validate_response_type(self, data: Any, expected_type: type) -> None:
        """
        ...
        if not isinstance(data, expected_type):
            error_msg = (
                f"Expected response type {expected_type.__name__} " f"but got {type(data).__name__}"
            )
            logger.error(error_msg)
            raise ValidationError(message=error_msg, response_body=data)
