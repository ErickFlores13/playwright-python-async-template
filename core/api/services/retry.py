...
from core.api.models import APIError, RetryExhaustedError
    ...
    async def execute(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        for attempt in range(1, self.config.max_attempts + 1):
            ...
            try:
                ...
                if hasattr(result, "status_code") and self._should_retry_status(
                    result.status_code, http_method, headers
                ):
                    logger.warning(
                        f"Attempt {attempt} failed with status {result.status_code}, retrying..."
                    )
                    last_exception = APIError(
                        f"Status {result.status_code}", status_code=result.status_code
                    )
                else:
                    if attempt > 1:
                ...
                if self._should_retry_exception(e):
                    logger.warning(
                        f"Attempt {attempt} failed with {type(e).__name__}: {str(e)}, retrying..."
                    )
                else:
        ...
        raise RetryExhaustedError(
            message=error_msg, status_code=getattr(last_exception, "status_code", None)
        )

    def _should_retry_status(
        self, status_code: int, method: str = "GET", headers: Optional[Dict[str, str]] = None
    ) -> bool:
        """
