...
def configure_logging(
    ...
    console_formatter = logging.Formatter(
        console_format or DEFAULT_LOG_FORMAT, datefmt="%Y-%m-%d %H:%M:%S"
    )
    ...
    if log_to_file:
        ...
        file_handler = logging.FileHandler(
            log_path / "test_execution.log", mode="a", encoding="utf-8"
        )
        file_handler.setLevel(level)
        file_formatter = logging.Formatter(
            file_format or FILE_LOG_FORMAT, datefmt="%Y-%m-%d %H:%M:%S"
        )
